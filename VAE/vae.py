...
#入力画像はこれらの2つのパラメーターにエンコードされる
z_mean = layers.Dense(latent_dim)(x)#(None, 2)
z_log_var = layers.Dense(latent_dim)(x)#(None, 2)
'''
#--------------------------------------------------------------
#空間サンプリング関数
def sampling(args):
    z_mean, z_log_var = target_size
    epsilon = K.random_normal(shape=(K.shape(z_mean)[0], latent_dim), mean=0., stddev=1.)
    return z_mean _ K.exp(z_log_var) * epsilon
...
class CustomVariationalLayer(keras.layers.Layer):
    def vae_loss(self, x, z_decoded):
        ...
        xent_loss = kerass.metrics.binary_crossentropy(x, z_decoded)
        kl_loss = -5e-4 * K.mean(
